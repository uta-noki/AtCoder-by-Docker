...
int main() {
    ...
    cin >> s;
    // s_i = s;
    // s = s_i.at(0) + s_i.at(1) + s_i.at(2) + s_i;
    int sz = s.size();
    vector<int> dp(sz,0);

    if (sz==1) {cout << 0 << "\n"; return 0;}
    if (sz==2) {
      if (s.at(0)!=s.at(1)) cout << 2 << "\n";
      else cout << 1 << "\n";
      return 0;
    }
    ...
    // 三文字目
    if (s.at(1)!=s.at(2) && dp.at(1)==2) dp.at(2) = 3;
    else dp.at(2) = 2;

    rep(i,3,sz) {
        if (s.at(i)!=s.at(i-1)) dp.at(i) = dp.at(i-1) + 1;
        else dp.at(i) = dp.at(i-3) + 2;
    }
