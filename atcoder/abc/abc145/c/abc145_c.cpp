...
int main() {
    ...
    vector<double> x(n,0), y(n,0), comb(n,0);
    // vector<double> dist;

    rep(i,1,n+1) n_comb *= i;

    rep(i,0,n) comb.at(i) = i;
    rep(i,0,n) cin >> x.at(i) >> y.at(i);

    double ans = 0;

    do {
      // rep(i,0,n) cout << comb.at(i) << " ";
      // cout << endl;
      rep(i,0,n-1) {
        double x_ij = (x.at(comb.at(i))-x.at(comb.at(i+1)))*(x.at(comb.at(i))-x.at(comb.at(i+1))), y_ij = (y.at(comb.at(i))-y.at(comb.at(i+1)))*(y.at(comb.at(i))-y.at(comb.at(i+1)));
        ans += sqrt(x_ij+y_ij);
      }
      // cout << dist.back() << " ";
    ...
    cout << setprecision(10) << (ans/n_comb) << "\n";
