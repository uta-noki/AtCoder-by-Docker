...
int main() {
    ...
    cin >> n >> k;
    vector<ll> a(n), b(n);
    vector<vector<bool>> check(2,vector<bool>(n,false));
    check.at(0).at(0) = true;
    check.at(1).at(0) = true;
    ...
    rep(i,0,n-1) {
        if (check.at(0).at(i)) {
            if (abs(a.at(i)-a.at(i+1))<=k) check.at(0).at(i+1) = true;
            if (abs(a.at(i)-b.at(i+1))<=k) check.at(1).at(i+1) = true;
        }
        if (check.at(1).at(i)) {
            if (abs(b.at(i)-b.at(i+1))<=k) check.at(1).at(i+1) = true;
            if (abs(b.at(i)-a.at(i+1))<=k) check.at(0).at(i+1) = true;
        }
    }

    cout << (check.at(0).at(n-1) || check.at(1).at(n-1) ? "Yes":"No") << "\n";
