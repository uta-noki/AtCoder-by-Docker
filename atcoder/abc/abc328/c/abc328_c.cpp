...
int main() {
    ...
    vector<int> ans(n,0);
    if (n>1) {
        if (s.at(0)==s.at(1)) ans.at(1) = 1;
    }

    rep(i,1,n) {
        if (s.at(i)!=s.at(i-1)) ans.at(i) = ans.at(i-1);
        else ans.at(i) = ans.at(i-1) + 1;
    }
    ...
    rep(i,0,q) {
        //sm = accumulate(next(ans.begin(),l.at(i)),next(ans.begin(),r.at(i)),0);
        //cout << sm << " ";
        cout << ans.at(r.at(i)-1) - ans.at(l.at(i)-1) << "\n";
    }
