...
int main() {
    ...
    // rep(i,0,n) cin >> a.at(i) >> c.at(i);
    rep(i,0,n) {
      int ai, ci;
      cin >> ai >> ci;
      cards.at(i) = {ai,ci,i+1};
    }

    sort(cards.begin(),cards.end(),[](const tuple<int,int,int> x,const tuple<int,int,int> y)
    {
    ...
    for (auto &card:cards) {
      // chmin(mn_cost,get<1>(card));
      if (mn_cost > get<1>(card)) {
        ans.push_back(get<2>(card));
        mn_cost = get<1>(card);
      }
    }
