...
int main() {
    ...
    int n, a_max, b_max;
    cin >> n;
    vector<int> a(n,0), q(n,0), b(n,0);

    rep(i,0,n) cin >> q.at(i);
    rep(i,0,n) cin >> a.at(i);
    rep(i,0,n) cin >> b.at(i);
    a_max = *max_element(q.begin(),q.end());
    b_max = *max_element(q.begin(),q.end());
    //cout << a_max << " " << b_max << "\n";
    int ans = *max_element(q.begin(),q.end());

    rep(i,0,n) {
      if (a.at(i)!=0) a_max = min(a_max,q.at(i)/a.at(i));
      else a_max = min(a_max,q.at(i));
      if (b.at(i)!=0) b_max = min(b_max,q.at(i)/b.at(i));
      else b_max = min(b_max,q.at(i));
      cout << a_max << " " << b_max << "\n";
      // ans = min(ans,a_max+b_max);
    }
    //cout << a_max << " " << b_max << "\n";
    cout << ans << "\n";
