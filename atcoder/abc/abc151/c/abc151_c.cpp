...
int main() {
    ...
    ll n, m, cnt_ac = 0, cnt_wa = 0;
    cin >> n >> m;
    vector<ll> p(m,0), wa(n,0);
    map<ll,bool> is_ac;
    vector<string> s(m);

    rep(i,0,m) cin >> p.at(i) >> s.at(i);

    rep(i,0,m) {
      if (!is_ac[p.at(i)] && s.at(i)=="AC") is_ac[p.at(i)] = true;
      if (!is_ac[p.at(i)] && s.at(i)=="WA") wa.at(p.at(i)-1)++;
    }

    for (auto v:is_ac) {
      if (v.second) {
        cnt_ac++;
        cnt_wa += wa.at(v.first-1);
      }
    }
