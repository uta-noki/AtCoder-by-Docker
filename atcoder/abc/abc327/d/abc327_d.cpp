...
int main() {
    ...
    rep(i,0,m) cin >> a.at(i), a.at(i)--;
    rep(i,0,m) cin >> b.at(i), b.at(i)--;
    rep(i,0,m) {
    ...
    auto dfs = [&](auto dfs, int v, int c = 0) -> void {
        if (color.at(v) != -1) return;
        color.at(v) = c;
        for (int u : to.at(v)) dfs(dfs, u, c^1);
    };

    rep(v,0,n) if (color.at(v) == -1) {
        dfs(dfs,v);
    }

    rep(i,0,m) if (color.at(a.at(i)) == color.at(b.at(i))) {
        cout << "No" << "\n";
        return 0;
    }
