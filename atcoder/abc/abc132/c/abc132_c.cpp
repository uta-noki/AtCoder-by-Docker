...
int main() {
    ...
    int n, ans = 0;
    cin >> n;
    vector<int> d(n,0), difficult(100001,0);
    rep(i,0,n) cin >> d.at(i);
    rep(i,0,n) difficult.at(d.at(i))++;

    rep(i,0,difficult.size()-1) difficult.at(i+1) += difficult.at(i);
    // rep(i,0,difficult.size()) cout << difficult.at(i) << " ";
    rep(i,1,100001) {
        if (difficult.at(i-1)==difficult.at(100000)-difficult.at(i-1)) ans++;
    }
