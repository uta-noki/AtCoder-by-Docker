...
int main() {
    ...
    rep(i,0,n) {
        if (i==0) ans = min({ans, a.at(n-1)-a.at(i), a.at(i)+k-a.at(i+1)});
        else if (i==n-1) ans = min({ans, a.at(i)-a.at(0), k-a.at(i)+a.at(i-1)});
        else ans = min({ans, k-a.at(i)+a.at(i-1), a.at(i)+k-a.at(i+1)});
    }
