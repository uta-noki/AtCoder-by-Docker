...
int main() {
    ...
    int h, w, cnt_init_wall = 0;
    cin >> h >> w;
    vector<string> s(h);
    vector<vector<int>> dist(h,vector<int>(w,-1));
    rep(i,0,h) cin >> s.at(i);
    rep(i,0,h) rep(j,0,w) if (s.at(i).at(j)=='#') cnt_init_wall++;
    ...
    while(!que.empty()) {
      ...
      int next_i, next_j;
      rep(i,0,4) {
        next_i = pa.first+di.at(i), next_j = pa.second + dj.at(i);
        if (next_i<0 || next_j<0 || next_i>=h || next_j>=w) continue;
        if (s.at(next_i).at(next_j)=='#' || dist.at(next_i).at(next_j)!=-1) continue;
        que.push({next_i, next_j});
        dist.at(next_i).at(next_j) = dist.at(pa.first).at(pa.second) + 1;
      }
    }

    if (dist.at(h-1).at(w-1)==-1) cout << -1 << "\n";
    else {
      cout << h*w-dist.at(h-1).at(w-1)-cnt_init_wall-2 << "\n";
    }
