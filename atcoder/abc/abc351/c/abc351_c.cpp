...
int main() {
    ...
    // rep(i,0,n) que.push(a.at(i));

    rep(i,0,n) {
      // int fi, se, th;
      // fi = que.front(), que.pop();
      // se = que.front(), que.pop();
      // if (ans.back()!=fi) {
      //   if (fi==se) ans.push(fi+1);
      //   else th = que.front(), que.pop();
      // }
      // ans.pop_back()
      
      que.push_back(a.at(i));
      if (que.size()<=1) continue;
      while(true) {
        int fi, se;
        fi = que.back(), que.pop_back();
        se = que.back(), que.pop_back();
        if (fi!=se) {
          que.push_back(se), que.push_back(fi);
          break;
        }
        else que.push_back(fi+1);
      }
    }
