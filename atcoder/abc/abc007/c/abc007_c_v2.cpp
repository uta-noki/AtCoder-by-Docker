...
int main() {
    ...
    // rep(i,0,r) rep(j,0,C) cin >> c.at(i).at(j);
    ...
    while(!que.empty()) {
      pair<int,int> pair_i = que.front();
      que.pop();
      rep(i,0,4) {
        int d_i = pair_i.first+di.at(i), d_j = pair_i.second+dj.at(i);
        if (d_i<0 || d_j<0 || d_i>=r || d_j>=C) continue;
        if (c.at(d_i).at(d_j)=='#') continue;
        if (dist.at(d_i).at(d_j)!=-1) continue;
        que.push({d_i,d_j});
        dist.at(d_i).at(d_j) = dist.at(pair_i.first).at(pair_i.second)+1;
      }
    }

    cout << dist.at(gy).at(gx) << "\n";
