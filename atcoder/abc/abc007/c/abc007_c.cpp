...
int main() {
    ...
    while(!que.empty()) {
      ...
      que.pop();
      rep(i,0,4) {
        int next_i = now.first + di.at(i), next_j = now.second + dj.at(i);
        if (next_i<0 || next_j<0 || next_i>=r || next_j>=C) continue;
        if (c.at(next_i).at(next_j)=='#') continue;
        if (dist.at(next_i).at(next_j)!=-1) continue;
        que.push({next_i, next_j});
        dist.at(next_i).at(next_j) = dist.at(now.first).at(now.second) + 1;
      }
    }
