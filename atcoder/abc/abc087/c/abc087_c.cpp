...
int main() {
    ...
    int n, ans = 0;
    cin >> n;
    vector<vector<int>> a(2,vector<int>(n,0)), sm(2,vector<int>(n,0));

    rep(i,0,2) rep(j,0,n) cin >> a.at(i).at(j);

    sm.at(0).at(0) = a.at(0).at(0);
    sm.at(1).at(n-1) = a.at(1).at(n-1);
    
    rep(i,1,n) {
      sm.at(0).at(i) = sm.at(0).at(i-1) + a.at(0).at(i);
      sm.at(1).at(n-i-1) = sm.at(1).at(n-i) + a.at(1).at(n-i-1);
    }
    // reverse(sm.at(1).begin(),sm.at(1).end());

    // rep(i,0,n) {
    //   cout << sm.at(0).at(i) << " " << sm.at(1).at(i) << "\n";
    // }
    rep(j,0,n) ans = max(ans,sm.at(0).at(j)+sm.at(1).at(j));
