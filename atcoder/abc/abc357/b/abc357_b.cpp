...
int main() {
    ...
    string s, mn = "abcdefghijklmnopqrstuvwxyz", mx = "ABCDEFGHIJKLMNOPQRLTUVWXYZ";
    cin >> s;
    int small = 0, big = 0, n = s.size();

  
    rep(i,0,n) {
      bool mn_check = false;
      rep(j,0,26) {
        if (s.at(i)==mn.at(j)) {small++; mn_check = true;}
      }
      if (!mn_check) big++;
    }
    ...
    if (big>small) {
      ...
      rep(i,0,n) {s.at(i) = toupper(s.at(i));}
      cout << s << "\n";
    }
    else {
      rep(i,0,n) {s.at(i) = tolower(s.at(i));}
      cout << s << "\n";
    }
