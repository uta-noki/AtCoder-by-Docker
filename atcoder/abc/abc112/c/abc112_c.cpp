...
int main() {
    ...
    int n;
    cin >> n;
    vector<int> x(n,0), y(n,0), h(n,0);
    rep(i,0,n) cin >> x.at(i) >> y.at(i) >> h.at(i);

    rep(cx,0,101) {
        int H = 0;
        rep(cy,0,101) {
            rep(k,0,n) if (h.at(k)!=0) H = h.at(k) + abs(cx - x.at(k)) + abs(cy - y.at(k));
            bool check = true;
            rep(k,0,n) {
                if (h.at(k) != max(H - abs(cx - x.at(k)) - abs(cy - y.at(k)), 0)) check = false;
            }
            if (check) {
                cout << cx << " " << cy << " " << H << "\n";
                return 0;
