...
int main() {
    ...
    cin >> s;
    vector<char> check;
    string check_mozi = "abcdefghijklmnopqrstuvwxyz";

    rep(i,0,check_mozi.size()) {
        bool check1 = false;
        rep(j,0,s.size()) if (check_mozi.at(i)==s.at(j)) check1 = true;
        if (!check1) check.push_back(check_mozi.at(i));
    }
    
    sort(check.begin(),check.end());

    if (check.size()==0) cout << "None" << "\n";
    else cout << check.at(0) << "\n";
