...
int main() {
    ...
    int n;
    cin >> n;
    vector<pair<int,int>> v_i(2);
    vector<int> v(n);
    vector<int> even_hist(100001,0), odd_hist(100001,0);
    vector<pair<int,int>> odd, even;
    //map<int,vector<pair<int,int>>> cnt[2];
    rep(i,0,n) {
        cin >> v.at(i);
        if (i%2==0) even_hist.at(v.at(i))++;
        else odd_hist.at(v.at(i))++;
    }

    rep(i,0,100001) {
        odd.push_back({odd_hist.at(i),i});
        even.push_back({even_hist.at(i),i});
    }
    ...
    if (odd.at(0).second != even.at(0).second) {
        cout << n - odd.at(0).first - even.at(0).first << "\n";
    }
    else {
        cout << n - max(odd.at(0).first + even.at(1).first,odd.at(1).first + even.at(0).first) << "\n";
    }
